    ...
    <div className="h-full max-w-2xl mx-auto overflow-y-auto p-4 pb-24 scrollbar-thin">
      ...
      {offers.length === 0 ? (
        <div className="mt-4 rounded-[14px] border border-dashed border-border p-7 text-center">
          ...
          <p className="mx-auto mt-1 max-w-[360px] text-[12px] leading-5 text-muted-foreground">请在创建行程时开启外部推荐，或前往推荐中心刷新美团结果。</p>
          <Button asChild size="sm" className="mt-3"><Link href="../offers">打开推荐中心</Link></Button>
        </div>
      ) : (
        <div className="mt-4 space-y-3">
          {offers.map((offer) => (
            <article key={offer.id} className="rounded-[14px] border border-border bg-surface p-4 shadow-xs">
              <div className="flex items-start justify-between gap-3"><div><div className="mb-1 flex gap-1.5"><Badge variant="outline" className="text-[10px]">美团</Badge><span className="text-[10px] text-muted-foreground">{offer.structured ? "结构化结果" : "原文推荐"}</span></div><h2 className="text-base font-semibold">{offer.title}</h2></div>{offer.priceLabel ? <p className="shrink-0 text-sm font-semibold">{offer.priceLabel}</p> : null}</div>
