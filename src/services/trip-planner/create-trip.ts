...
/**
 * Trip creation pipeline shared by every entry point. The Web route, the skill
 * runtime and future adapters must not reimplement candidate grouping, day
 * assembly or outline compilation; they call this service and persist the
 * resulting trip through their own repository boundary.
 */
...
function ruleOutline(body: CreateTripWebRequest, candidates: Candidate[]): Outline {
...
async function llmOutlineSafe(body: CreateTripWebRequest, candidates: Candidate[], social?: LiveSocialSearchResult): Promise<Outline | undefined> {
  try {
...
/** Fetches social evidence for the destination; failures degrade silently. */
export async function collectSocialForTrip(body: CreateTripWebRequest): Promise<LiveSocialSearchResult | undefined> {
  if (process.env.NODE_ENV === "test" || process.env.VOYAGE_SOCIAL_ENABLED === "0") return undefined;
  try {
    return await searchTravelSocial({ city: body.destination, query: body.prompt });
  } catch {
    return undefined;
  }
}

export async function planTripFromRequest(body: CreateTripWebRequest): Promise<{
  source: "llm" | "rules";
  ...
  try {
    const built = await buildCandidates(body.destination);
    candidates = built.places;
    provider = built.provider;
  } catch (error) {
    throw error instanceof Error ? error : new Error("candidate lookup failed");
  }

  const social = await collectSocialForTrip(body);
  const llmEnv = process.env.LLM_BASE_URL;
  const outline = llmEnv ? await llmOutlineSafe(body, candidates, social) : undefined;
  const trip = await assembleTrip({ outline: outline ?? ruleOutline(body, candidates), body, candidates, social });
  return { source: outline ? "llm" : "rules", mapProvider: provider, trip, social };
}
